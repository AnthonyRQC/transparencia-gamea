...
import PlazoBadge from '../Card/PlazoBadge';
import TipoDenunciaBadge from '../Card/TipoDenunciaBadge';
import TabSolicitudes from '../Tabs/TabSolicitudes';
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-lg flex flex-col">
        <SheetHeader className="shrink-0 pb-2 border-b border-border/60">
          <div className="flex items-center gap-2 flex-wrap pr-6">
            <SheetTitle className="font-mono text-xl font-bold tracking-tight">{denuncia.ticket}</SheetTitle>
        ...
        {children ? (
          <div className="shrink-0 pt-2 pb-1 border-t border-border bg-card">
            {children}
          </div>
        ) : onAbrirArchivos ? (
          <div className="shrink-0 pt-2 pb-1 border-t border-border bg-card flex justify-end">
            <button
              type="button"
              onClick={() => onAbrirArchivos(denuncia.ticket)}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted text-foreground text-xs font-semibold hover:bg-accent transition-colors border border-border"
            >
