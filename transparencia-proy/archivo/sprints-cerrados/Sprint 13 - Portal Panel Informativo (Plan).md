...
|---|---|---|
| **13.1 Muro público + catálogos** | Sección en Welcome bajo consulta (info/preguntas al final): grid 2 col (aside 25% sticky + feed 1 col), filtros tipo (lista radial, sin conteos) + texto + fechas server-side, `Paginacion server` (10 por página), botón en `/seguimiento` ("ver avisos de mi caso" → `?buscar=` parcial, sin PIN). Por defecto últimos 12 meses + "Ver historial completo" (muro permanente). Búsqueda FULLTEXT MySQL (LIKE en SQLite/cortos). + botón en `/seguimiento` ("ver avisos de mi caso" → `?buscar=` parcial, sin PIN). Migración + seed `tipos_publicacion` (7) y `prioridades_publicacion` (3) + 2 pestañas en Panel Catálogos | `R1.1 semantica` (chips/badges), `R1.2 fechas`, `R1.4 Paginacion`, `ListaVacia` |
